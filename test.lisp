...
(progn
  ...
  (check '(lambda x (y z))
         '(lambda a (b c)))
  (check-not '(lambda x (y z))
             '(lambda a (b b)))
  (check `(,not. (,zerop. ,zero.))
         false.)
  (check `((,eq. (,inc. ,one.)) ,two.)
         true.)
  (check `(,zerop. (,dec. ,one.))
         true.)
  (check `(((,if. ,true.) ,one.) ,two.)
         one.)
  (check `(((,if. ,false.) ,one.) ,two.)
         two.)
  (check `((,leq. ,five.) ,one.)
         false.)
  (check `(,fac. (,inc. ,two.))
         `((,add. ,two.) ((,mul. ,two.) ,two.)))
  (check `((,eq. ((,pow. ,two.) ,three.)) ,eight.)
         true.)
  (check `((,eq. (,fib. ,five.)) ,eight.)
         true.)
  (check `(,len. ((,cons. ,one.) ((,cons. ,two.) ((,cons. ,three.) ,nil.))))
         three.)
  (check `((,append.
